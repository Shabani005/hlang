...
int main(int argc, char **argv) {
    ...
    // char* read = bytecode_buf;
    // we dont need null terminated string to use tokenize all, we can make a separate function to place
    // null terminator at bytecode_len so we can use functions like strlen() on it
    //printf("INPUT: %s\n", read);
